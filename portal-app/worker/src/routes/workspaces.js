 ...
 * are what close the race itself (ADR-0006).
 */
...
export async function handleDeleteWorkspace(request, env, user, params) {
  ...
  try {
    ...
    //
    // The lock claimed above (ADR-0006) closes the upload/replace-vs-delete
    // race this snapshot used to be exposed to: any upload/replace that
    // starts after the lock is claimed is rejected before it can write a new
    // r2_key this snapshot wouldn't see.
    ...
    for (let i = 0; i < r2Keys.length; i += 1000) {
      try {
        ...
        throw errorResponse('Failed to delete workspace files from storage; nothing was deleted — retry', 500)
      }
    }
    ...
    // failed R2 batch-delete (thrown as a Response above) or a failed D1
    // batch must not leave the workspace permanently locked out of future
    // uploads (ADR-0006).
